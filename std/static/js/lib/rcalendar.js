...
function rcalendar(text, mode, retfunction) { //文本域对象, 模式(dateonly,full), 选择日期后的事件函数(函数是新时间的Date对象)
	...
	if (!div_rcalendar) { //如果不存在，则初始化创建它
		...
		if (document.compatMode == "BackCompat" && navigator.userAgent.indexOf("MSIE") != -1) {
			...
			css += "#rmonths {background:#B0B0B0; border:1px solid #999999; border-top:0px; color:#FFFFFF; font-weight:bold; text-align:center; width:80px; height:100px; overflow:hidden;}";
			...
			css += "#ryear_add {padding:0px; height:12px; width:36px; overflow:hidden; cursor:pointer;}";
			css += "#rmonths {background:#B0B0B0; border:1px solid #999999; border-top:0px; color:#FFFFFF; font-weight:bold; text-align:center; width:78px; height:100px; overflow:hidden;}";
  ...
  else if (rmode == "timeonly") {
    document.getElementById("rtime").style.visibility = "visible";
		document.getElementById("rcalendar_ym").style.visibility = "hidden";
		document.getElementById("rweeks").style.visibility = "hidden";
		document.getElementById("rdates").style.visibility = "hidden";
	} else
...
function rfill_ryears(year) {
	...
	str += '<div class="ryear" onmouseover="this.style.background=\'#FFFFFF\';this.style.color=\'#666666\';" onmouseout="this.style.background=\'#B0B0B0\';this.style.color=\'#FFFFFF\';" onclick="rset_year(this.innerHTML)">' + 'unk' + '</div>';
...
function rfill_rmonths() {
	...
	str += '<div class="rmonth" onmouseover="this.style.background=\'#FFFFFF\';this.style.color=\'#666666\';" onmouseout="this.style.background=\'#B0B0B0\';this.style.color=\'#FFFFFF\';" onclick="rset_month(this.innerHTML)">' + 'unk' + '</div>';
...
function rfill_rhours() {
	...
	str += '<div class="rhour" onmouseover="this.style.background=\'#E9E9E9\';this.style.color=\'#000000\';" onmouseout="this.style.background=\'#B0B0B0\';this.style.color=\'#FFFFFF\';" onclick="rset_hour(this.innerHTML)">' + 'unk' + '</div>';
...
function rfill_rminutes() {
	...
	str += '<div class="rminute" onmouseover="this.style.background=\'#E9E9E9\';this.style.color=\'#000000\';" onmouseout="this.style.background=\'#B0B0B0\';this.style.color=\'#FFFFFF\';" onclick="rset_minute(this.innerHTML)">' + 'unk' + '</div>';
...
function rset_datetime(d, m_add) {
  ...
  if (oi && !i) { i = oi; }
	...
	if (rmode == "full")
		rtext_date.value = year + "-" + m + "-" + d + " " + h + ":" + i + ":" + s;
  else if (rmode == "timeonly")
		rtext_date.value = h + ":" + i + ":" + s;
	else
		rtext_date.value = year + "-" + m + "-" + d;
	rcalendar_close();
