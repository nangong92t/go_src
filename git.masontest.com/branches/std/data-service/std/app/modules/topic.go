...
// 获取某Topic下的comment列表.
func (c *Topic) GetCommentList(tid int64, page int, limit int, needTotal bool) (*Pager, error) {
    comments    := []models.Comment{}
    if page == 0 { page = 1 }
    if limit == 0 { limit = 20 }
    offset := (page-1) * limit

    sql := "select * from comment where %s order by created desc limit ?,?"
    where := "is_del=0"
    if tid != 0 { where = where + " and topic_id=" + strconv.FormatInt(tid, 10) }
    c.Db.Select(&comments, fmt.Sprintf(sql, where), offset, limit)

    total       := int64(0)
    if needTotal {
        total, _   = c.Db.SelectInt(fmt.Sprintf("select count(1) from comment where %s", where))
    }
...
func (m *Topic) RemoveComment(cid []interface{}) (bool, error) {
    valLen      := len(cid)
    condition   := fmt.Sprintf("comment_id in (%s)",  strings.TrimRight(strings.Repeat("?,", valLen), ","))
    sql         := "delete from comment where %s"
    _, err      := m.Db.Exec(fmt.Sprintf(sql, condition), cid...)
