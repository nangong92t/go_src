...
return array(
    ...
    'workers' => array(
        ...
        'Vehicle' => array(
            ...
            'bootstrap'             => '../../../vehicle/Service/init.php', // 进程初始化时调用一次，可以在这里做些全局的事情，例如设置autoload
        ),
        ...
        'User' => array(
            ...
            'max_requests'          => 1000,     // [选填]进程接收多少请求后退出              不配置默认是0，不退出
            'worker_class'          => 'JmTextWorker',// worker使用的类
            'bootstrap'             => '../../../user/Service/init.php', // 进程初始化时调用一次，可以在这里做些全局的事情，例如设置autoload
        ),

        // LY Resource 核心资源功能模块
        'ResourceService' => array(
            'protocol'              => 'tcp',    // [必填]tcp udp
            'port'                  => 2003,     // [必填]监听的端口
            'child_count'           => 10,       // [必填]worker进程数 注意:每个进程大概占用30M内存，总worker数量不要超过800个
            'ip'                    => "0.0.0.0",// [选填]绑定的ip，注意：生产环境要绑定内网ip 不配置默认是0.0.0.0
            ...
            'max_requests'          => 1000,     // [选填]进程接收多少请求后退出              不配置默认是0，不退出
            'worker_class'          => 'JmTextWorker',// worker使用的类
            'bootstrap'             => '../../../ResourceService/Service/init.php', // 进程初始化时调用一次，可以在这里做些全局的事情，例如设置autoload
        ),
        ...
        'BasicService' => array(
            ...
            'max_requests'          => 1000,     // [选填]进程接收多少请求后退出              不配置默认是0，不退出
            'worker_class'          => 'JmTextWorker',// worker使用的类
            'bootstrap'             => '../../../BasicService/Service/init.php', // 进程初始化时调用一次，可以在这里做些全局的事情，例如设置autoload
        ),
        ...
        'OrderService' => array(
            ...
            'max_requests'          => 1000,     // [选填]进程接收多少请求后退出              不配置默认是0，不退出
            'worker_class'          => 'JmTextWorker',// worker使用的类
            'bootstrap'             => '../../../OrderService/Service/init.php', // 进程初始化时调用一次，可以在这里做些全局的事情，例如设置autoload
        ),
