...
func GenerateEqualOuterQuery(sel *sqlparser.Select, tableInfo *schema.MysqlTable) *sqlparser.ParsedQuery {
	buf := sqlparser.NewTrackedBuffer(nil)
...
func GenerateInOuterQuery(sel *sqlparser.Select, tableInfo *schema.MysqlTable) *sqlparser.ParsedQuery {
	buf := sqlparser.NewTrackedBuffer(nil)
	fmt.Fprintf(buf, "select ")
	writeColumnList(buf, tableInfo.Columns)
...
func GenerateSelectSubquery(sel *sqlparser.Select, tableInfo *schema.MysqlTable, index string) *sqlparser.ParsedQuery {
...
func GenerateUpdateSubquery(upd *sqlparser.Update, tableInfo *schema.MysqlTable) *sqlparser.ParsedQuery {
	return GenerateSubquery(
		tableInfo.Indexes[0].Columns,
...
func GenerateDeleteSubquery(del *sqlparser.Delete, tableInfo *schema.MysqlTable) *sqlparser.ParsedQuery {
	return GenerateSubquery(
		tableInfo.Indexes[0].Columns,
