...
import (
    ...
    "strconv"
    "runtime"
    "git.masontest.com/branches/goserver/workers"
...
func (s *GoServer) createTcpSocketsAndListen(protocol, ip, port string, config *map[string]string) bool {
    ...
    for i:=1; i<=childCount; i++ {
        go func() {
            switch workerClass {
                case "RpcWorker":
                    workers.NewRpcWorker(i, s.MainCfg["rpc_secret_key"], config, listener, chSync).Run()
                default:
                    workers.NewEchoWorker(i, listener, chSync).Run()
            }
        }()
    }
    for i:=1; i<=childCount; i++ {
...
func (s *GoServer) createUdpSocketsAndListen(protocol, ip, port string, config *map[string]string) bool {
    // 创建监听
    laddr, err := net.ResolveUDPAddr("udp4", ip+":"+port)
    if nil != err {
        s.Log.Add("add port error in UDP service: ", err)
        return false
    }
    listener, err := net.ListenUDP("udp4", laddr)
    if nil != err {
        s.Log.Add("add listener error in UDP service: ", err)
        return false
    }
    defer listener.Close()
    s.Log.Add((*config)["name"] + " UDP service start")
