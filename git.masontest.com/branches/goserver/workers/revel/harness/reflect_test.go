...
func TestProcessBookingSource(t *testing.T) {
	revel.Init("prod", "git.masontest.com/branches/goserver/workers/revel/samples/booking", "")
	sourceInfo, err := ProcessSource([]string{revel.AppPath})
	if err != nil {
		t.Fatal("Failed to process booking source with error:", err)
	}

	CONTROLLER_PKG := "git.masontest.com/branches/goserver/workers/revel/samples/booking/app/controllers"
...
func BenchmarkProcessBookingSource(b *testing.B) {
	revel.Init("", "git.masontest.com/branches/goserver/workers/revel/samples/booking", "")
